...
#### 장비 슬롯 구성
```
┌── Primary Weapon ──── (자동 공격용 주무기)
├── Secondary Weapon ── (보조 공격, 특성 다름)
├── Traversal Tool ──── 그래플링 훅 (고정, Scout 정체성)
└── Support Tool ────── 조명탄 (고정, 시야 확보)
```
...
| 필살기 | Nitra 비용 | 효과 |
|--------|-----------|------|
| **Supply Drop** | 40 Nitra | 체력 회복 + 탄약 보급, 착지 시 광역 데미지 |
| **Resupply Pod** | 80 Nitra | 대형 보급, 전체 회복 + 일시적 무적 |

> DRG 원작에서 Nitra 80개 = Supply Pod 호출과 동일한 컨셉을 유지하면서,
> 필살기라는 게임적 장치로 변환.

### 4-3. Pit Jaw 환경 위험 시스템
- DRG의 환경 위험 요소를 게임에 도입
- **Pit Jaw**가 맵 곳곳에 **낮은 확률**로 숨어 있음
- 평소에는 보이지 않거나 미세한 힌트만 표시
...
Pit Jaw 배치 예시:
  ...
  │        ▲ Scout       │
  │     ╔═══╗            │
  │     ║PIT║ -25 HP!    │  ← Pit Jaw 출현 + 데미지
  │     ║JAW║            │
  │     ╚═══╝            │
  └──────────────────────┘
```

- **전략적 의미**: 무작정 돌진 방지, 주변 관찰 유도
- 조명탄 사용 시 Pit Jaw 위치 미리 확인 가능 (Scout 고유 시너지)
...
- **필살기 버튼**: 화면 오른쪽 하단 (Nitra 충분 시 활성화)
- 레벨업 시: 게임 일시정지 + 3개 선택지 터치
- Nitra 채굴: 채굴 포인트 위에 서 있으면 자동 채굴 (별도 입력 없음)
